...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for(int xx = floor(xmin); xx <= ceil(xmax); xx++){
        ...
        {
            if(insideTriangle((float)xx+0.5, (float)yy+0.5, t.v)){
                auto[alpha, beta, gamma] = computeBarycentric2D((float)xx+0.5, (float)yy+0.5, t.v);
                float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                zp *= Z;
                if(zp < depth_buf[get_index(xx, yy)]){
                    ...
                    depth_buf[get_index(xx, yy)] = zp;
                    set_pixel(Eigen::Vector2i(xx, yy), pixel_color);
                }
            }
        }
